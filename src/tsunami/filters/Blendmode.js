...
*/
	...
	c.overlay = function(sourcePixels, targetPixels) {
		...
		c.overlay = function(src, target) {
			...
			for (var x = 0; x < target.width; x++) {
				for (var y = 0; y < target.height; y++) {
					...
					target.data[px]   = f1*tsunami.filters.Blendmode.Foverlay(r1,r2) + f2*r1 + f3*r2;
					target.data[px+1] = f1*tsunami.filters.Blendmode.Foverlay(g1,g2) + f2*g1 + f3*g2;
					target.data[px+2] = f1*tsunami.filters.Blendmode.Foverlay(b1,b2) + f2*b1 + f3*b2;
